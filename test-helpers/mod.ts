...
class TestRunner {
  #tests: Record<string, Deno.TestDefinition[]> = {};
  #focused = false;
  ...
  addTest(test: Deno.TestDefinition, group: string) {
  ...
  testWrapper(
    test: Deno.TestDefinition,
    _beforeEach: HookPromise,
    _afterEach: HookPromise,
  ) {
    Deno.test({
      ...test,
      fn: async (t) => {
...
export const focus = (
  ...
  testRunner.addTest({ name, fn, only: true }, groupCursor);
};
